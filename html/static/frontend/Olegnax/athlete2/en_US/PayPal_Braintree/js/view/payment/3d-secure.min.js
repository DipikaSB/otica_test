define(['jquery','PayPal_Braintree/js/view/payment/adapter','Magento_Checkout/js/model/quote','mage/translate','braintreeThreeDSecure','Magento_Checkout/js/model/full-screen-loader','PayPal_Braintree/js/helper/remove-non-digit-characters','PayPal_Braintree/js/model/vault-enabler'],function($,braintree,quote,$t,threeDSecure,fullScreenLoader,removeNonDigitCharacters,vaultEnablerModel){'use strict';return{config:null,setConfig:function(config){this.config=config;this.config.thresholdAmount=parseFloat(config.thresholdAmount);vaultEnablerModel.setPaymentCode(this.config.ccVaultCode);},getCode:function(){return'three_d_secure';},checkBillingLineLengths:function(errorMessage,billingAddress,shippingAddress){let lineError=null;if(billingAddress.street[0].length>50||shippingAddress.street!==undefined&&shippingAddress.street[0].length>50){lineError='line1';}else if(billingAddress.street[1].length>50||shippingAddress.street!==undefined&&shippingAddress.street[1].length>50){lineError='line2';}
...
return false;},getIpAddress:function(){return this.config.ipAddress;}};});