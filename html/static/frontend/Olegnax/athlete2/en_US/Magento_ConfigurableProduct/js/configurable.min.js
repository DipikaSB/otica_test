...
_.extend(option,{childSettings:childSettings.slice(),prevSetting:settings[index-1],nextSetting:settings[index+1]});childSettings.push(option);}},_configureForValues:function(){if(this.options.values){this.options.settings.each($.proxy(function(index,element){var attributeId=element.attributeId;element.value=this.options.values[attributeId]||'';this._configureElement(element);},this));}},_configure:function(event){event.data._configureElement(this);},_updatePriceDiff:function(pricebox){const $widget=this;const $wrap=$('.product-info-main, .product-bar__info-price');if(!pricebox.length||!$wrap.length){return;}
pricebox.on('priceUpdated',function(event,result){if(typeof result==='undefined'){return;}
let priceDiff=0;if(!$widget.priceDiffElement){let $el=$wrap.find('.ox-sale-price-dif');if($el.length){$widget.priceDiffElement=$el;}else{$widget.priceDiffDisabled=true;}}
if(!$widget.priceDiffDisabled&&$widget.priceDiffElement?.length){$widget.priceDiffAsPercent=$widget.priceDiffElement.hasClass('as-percent');if($widget.priceDiffAsPercent){priceDiff=Math.round((1-result.finalPrice?.amount / result.oldPrice?.amount)*100);}else{priceDiff=result.oldPrice?.amount-result.finalPrice?.amount;}
$widget._togglePriceDiff($widget.priceDiffElement,priceDiff,$widget.priceDiffAsPercent);}});},_togglePriceDiff:function($el,price,asPercent){if(!$el){return;}
const $priceDiffEl=$el.find('.js-ox-price-diff');if(0>=price||!$priceDiffEl.length){$el.addClass('d-none');}else{if(asPercent){$priceDiffEl.html(price);}else{this._formatedPriceDiff($priceDiffEl,price);}
$el.removeClass('d-none');}},_formatedPriceDiff:function($el,price){let formatedPrice=priceUtils.formatPrice(price,this.options.priceFormat);$el.html(formatedPrice);},_updateSku:function(){const $sku_wrapper=$('.product-info-stock-sku .sku');if($sku_wrapper.length){if(!$sku_wrapper.data('sku')){$sku_wrapper.data('sku',$sku_wrapper.find('.value').text());}
