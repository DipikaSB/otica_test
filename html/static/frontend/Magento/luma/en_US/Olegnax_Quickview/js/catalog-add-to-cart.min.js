...
self.enableAddToCartButton(form);window.parent.ajaxCartTransport=!window.OXAjaxNShowMCart;},error:function(res){$(window.parent.document).trigger('ajax:addToCart:error',{'sku':form.data().productSku,'productIds':productIds,'form':form,'response':res});self.enableAddToCartButton(form);},complete:function(res){if(res.state()==='rejected'){window.parent.location.reload();}}});},_redirect:function(url){var urlParts,locationParts,forceReload;urlParts=url.split('#');locationParts=window.location.href.split('#');forceReload=urlParts[0]===locationParts[0];window.parent.location.assign(url);if(forceReload){window.parent.location.reload();}},});return $.mage.OxQuickviewcatalogAddToCart;});