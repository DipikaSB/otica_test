define(['jquery','braintree','braintreeDataCollector','braintreeApplePay','mage/translate','Magento_Checkout/js/model/payment/additional-validators','PayPal_Braintree/js/helper/check-guest-checkout'],function($,braintree,dataCollector,applePay,$t,additionalValidators,checkGuestCheckout,){'use strict';return{init:function(element,context){if(!element||!context){return;}
...
if(typeof context.onButtonClick==='function'){context.onButtonClick(session,this,e);}else{$('body').loader('hide');session.begin();}}catch(err){$('body').loader('hide');console.error('Braintree ApplePay Unable to create ApplePaySession',err);this.showError();return false;}}.bind(this));element.appendChild(el);},showError(){alert($t('We\'re unable to take payments through Apple Pay at the moment. Please try an alternative payment method.'));}};});