define(['jquery','mage/storage','Magento_Checkout/js/model/quote','Magento_Checkout/js/model/url-builder','Magento_Customer/js/model/customer'],function($,storage,quote,urlBuilder,customer){'use strict';return{canSave:true,paymentMethod:null,quoteId:quote.getQuoteId(),isLoggedIn:customer.isLoggedIn(),saveUrl:'',initialize:function(){quote.paymentMethod.subscribe(function(method){this.paymentMethod=null;if(method){this.paymentMethod=method.method;}},this);if(this.isLoggedIn){this.saveUrl=urlBuilder.createUrl('/checkout/saveInsertedInfo',{});}else{this.saveUrl=urlBuilder.createUrl('/checkout/:cartId/saveInsertedInfo',{cartId:this.quoteId});}
...
storage.post(this.saveUrl,JSON.stringify(request),false);}};});