...
define([
    ...
    const MAPPING_BLOCK_NAME = {
        ...
        BLOCK_UNIQUE_CLASSES = {
            shipping_address: 'checkout-shipping-address',
            shipping_method: 'checkout-shipping-method',
            payment_method: 'checkout-payment-method',
            summary: 'checkout-summary',
            delivery: 'checkout-delivery',
            additional_checkboxes: 'checkout-additional-checkboxes'
        };
    ...
    return {
        ...
        getCheckoutBlock: function (blockName) {
            ...
            if (blockName === 'shipping_method') {
                ...
                console.log(configValues.shipping_disable);
                if (configValues.shipping_disable == 1) {
                    return null;
                } else {
                    blockPath = 'checkout.steps.shipping-step.shippingAddress';
                }
            } else {
                blockPath = MAPPING_BLOCK_NAME[blockName];
            }
        
            var requestComponent = this.checkoutBlocks[blockName]
                || this.requestComponent(blockPath);
        ...
        getBlockClassNames: function (blockName) {
            ...
            // Add unique class if defined
            var configValues = window.checkoutConfig || {};
            if (configValues.payment_disable == 1) {
                if (BLOCK_UNIQUE_CLASSES[blockName]) {
                    classNames += ' ' + BLOCK_UNIQUE_CLASSES[blockName];
                }
            }
