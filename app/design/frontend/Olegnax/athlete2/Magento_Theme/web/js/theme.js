...
require(["jquery"], function ($) {
    ...
    $(function () {
        ...
        // Focus when clicking search icon
        $(document).on("click", ".action.search, .header__search-toggle, .ox-search-toggle", function () {
            setTimeout(function () {
                $(".js-input-focus").first().trigger("focus");
            }, 200);
        });

        // Focus when search overlay opens
        $('body').on('modal_opened.OXmodal', function () {
            const $searchModal = $(".ox-slideout-top");
            if ($searchModal.length && $searchModal.hasClass("active")) {
                setTimeout(function () {
                    $(".js-input-focus").first().trigger("focus");
                }, 200);
            }
