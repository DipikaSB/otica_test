...
abstract class AbstractSocial extends Action
{
    ...
    protected function _loginPostRedirect()
    {
        ...
        $requestedRedirect = $this->_url->getUrl('customer/account');
        ...
        if ($this->_request->getParam('authen') === 'popup') {
           ...
           if (is_string($requestedRedirect) && strpos($requestedRedirect, 'checkout/cart') !== false) {
            $url = $this->_url->getUrl('checkout');
            $this->accountRedirect->clearRedirectCookie();
            }
            elseif (is_string($requestedRedirect) && strpos($requestedRedirect, 'walletsystem/index/index') !== false) {
            $url = $this->_redirect->success($requestedRedirect);
            $this->accountRedirect->clearRedirectCookie();
            }
            elseif ($requestedRedirect && !$this->apiHelper->getConfigValue('customer/startup/redirect_dashboard')) {
                $url = $this->_redirect->success($requestedRedirect);
                $this->accountRedirect->clearRedirectCookie();
            }
        }       
    ...
    public function _appendJs($content = null, $customerToken = null)
    {
        ...
        if ($redirectUrl) {
            // Send loginRedirect event with the correct redirect URL
            $script = $this->apiHelper->generateBroadcastChannelScript('loginRedirect', [
                'redirectUrl' => $redirectUrl
            ]);

            // If it’s a wallet redirect or checkout, open in parent window
            if (strpos($redirectUrl, 'walletsystem/index/index') !== false 
                || strpos($redirectUrl, 'checkout') !== false) {
                $script .= "<script>
                ...
                // Normal redirect handled by broadcast channel
            ...
            $data  = $customerToken ? ['customerToken' => $customerToken] : [];
            $script = $this->apiHelper->generateBroadcastChannelScript($event, $data);

            if ($customerToken) {
                $script .= "<script>window.MP_ACCESS_TOKEN_KEY = '{$customerToken}';</script>";
            } else {
    ...
    protected function setBodyResponse($message)
    {
        ...
        // Add JS to close the popup or redirect
        $content .= <<<JS
        <script type="text/javascript">
            window.close(); // Attempt to close the popup
            window.opener && window.opener.location.reload(); // Reload opener (if needed)
        </script>
        JS;
