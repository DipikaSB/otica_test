...
class Items extends \Magetrend\PdfTemplates\Model\Pdf\Element\Table
{
     ...
     * @return void
     */
    public function drawTaxRow(&$topY, $index)
    {
        $source = $this->getSource();
        $order = $this->getOrder();
        $taxAmount = (float)$source->getTaxAmount();
        ...
        if ($shippingAddress) {
            ...
            if (empty($regionName)) {
                $regionName = (string)$shippingAddress->getRegionCode();
            }
        }

        $isGujarat = (strpos(strtolower($regionName), 'gujarat') !== false || strtoupper($regionName) == 'GJ');
        $label = $isGujarat ? 'OUTPUT CGST-SGST' : 'OUTPUT IGST';
        $formattedTax = $order->formatPriceTxt($taxAmount);
        ...
        $paddingRaw = $this->getRowPadding();
        $padding = [
            $this->removePx($paddingRaw[0] ?? 0),
            $this->removePx($paddingRaw[1] ?? 0),
            $this->removePx($paddingRaw[2] ?? 0),
            $this->removePx($paddingRaw[3] ?? 0)
        ];
        ...
        foreach ($columnConfig as $key => $column) {
            ...
            $columnWidth = $this->getColumnWidth($key, false, false);
            ...
            $columnsData[$key] = [
                'padding' => $padding,
                'height' => $padding[0] + count($value) * $lineHeight + $padding[2],
                'text' => [
                    $key => [
                        'text' => $value,
                        'font' => $fontCode,
        ...
        // Check space
        if (!$this->isEnoughSpaceForItem($rowHeight, $topY, true)) {
            // Ideally we should handle page break here, but for a single total row it's complex.
            // We'll just draw it and let the next page logic handle it if it was a false return.
            // However, drawItems already returned true, so we are on the last page of items.
        }

        $this->drawRowBackground($index, $topY, $rowHeight);
        $this->drawRowHorizontalBorder($index, $topY, $rowHeight);
        $this->drawRowVerticalBorder($topY, $rowHeight);
        $this->drawRowText($columnsData, $topY);
