...
class Edit extends \Magento\Backend\Block\Widget\Form\Container
{
    ...
    {
        $this->_objectId = 'magicslider_id';
        $this->_blockGroup = 'Setblue_Magicslider';
        $this->_controller = 'adminhtml_magicslider';

        parent::_construct();

        $this->buttonList->update('save', 'label', __('Save Magicslider'));
        $this->buttonList->update('delete', 'label', __('Delete'));

        if ($this->getRequest()->getParam('current_magicslider_id')) {
            ...
            if ($magicsliderId = $this->getRequest()->getParam('magicslider_id')) {
                $this->_formScripts[] = '
					window.magicslider_id = '.$magicsliderId.';
				';
            }
        } else {
            $this->buttonList->add(
