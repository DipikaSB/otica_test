...
class Code extends \Magento\Framework\View\Element\Template
{
    ...
    private function checkDisableMore($action, $listDisableCode)
    {
        ...
        if ($action == 'checkout_cart_index' && in_array('cart_page', $listDisableCode)) {
            return 404;
        }
    ...
    /**
     * Get cart data for ViewCart event
     *
     * @return false|int|string
     * @throws \Magento\Framework\Exception\NoSuchEntityException
     */
    public function getViewCart()
    {
        ...
        $data   = $this->getFacebookPixelData();
        $action = $data['full_action_name'];
        if ($action == 'checkout_cart_index' && $this->helper->isViewCart()) {
            ...
            if (!empty($items)) {
                ...
                $cartData = [
                    ...
                    'value' => number_format(
                        $checkout->getQuote()->getGrandTotal(),
                        2,
                        '.',
                        ''
                    ),
                ...
                $viewCartData = $this->helper->serializes($cartData);
            }
        }
        return $viewCartData;
    }
