...
class Validate extends \Magento\Backend\App\Action
{
    ...
    public function validateCsv()
    {
        ...
        foreach ($data as $rowNumber => $rowData) {
            ...
            if (
                trim($rowData[0] ?? '') === '' || // Store View
                trim($rowData[1] ?? '') === '' || // Product ID
                trim($rowData[4] ?? '') === '' || // Nick Name
                trim($rowData[5] ?? '') === '' || // Review Summary
                trim($rowData[6] ?? '') === '' || // Review Detail
                trim($rowData[8] ?? '') === ''    // Rating
            ) {
